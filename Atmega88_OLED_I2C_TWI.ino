...
#define oled 0x3c 
 ...
 byte sda=1<<4, scl=1<<5;  //A4 A5 of PORTC

void setup(void) {
  Serial.begin(9600);
  pinMode(A4,INPUT_PULLUP); 
  pinMode(A5,OUTPUT);   //not a real i2c bus, master transmit only
  for (byte j=0; j<sizeof(initcode); j++) { twiWrite(initcode[j],0); }
...
tim=millis();
for (int c=0; c<10000; c++) twiWrite(0,0); 
Serial.print("Time ");Serial.println(millis()-tim);
}

void displayon(void) {
  twiWrite(0x21,0);twiWrite(0x20,0);twiWrite(0x20 + 64 - 1,0);
  twiWrite(0x22,0);twiWrite(0,0); // Page start address 
  twiWrite((48 / 8) - 1,0); // Page end address
  
    for (uint16_t i=0; i<384; i++)  twiWrite(bufr[i],0x40);  
}
// =============================== i2c routines =================================

void twi8bit( unsigned char Twid )                        //Sends 8 Bits & Ack
{
  /*
 unsigned char Mask=0x80;
 do {    
  if (Twid & Mask) PORTC |= sda;  else  PORTC &= ~sda;   //bit set / not set
  Mask >>= 1;                                             //Shift Mask
  PORTC |= scl;  PORTC &= ~scl;                           //clock 
 } while( Mask );                                         // Loop Until Mask = 0
 */
 for (byte msk=0x80; msk>0; msk>>=1) { 
  if (Twid & msk) PORTC |= sda; else PORTC &= ~sda;      //bit set / not set
  PORTC |= scl;  PORTC &= ~scl;                          //clock 
  }
 
 DDRC &= ~sda;                                     //slave sends pulse: port input
 PORTC |= scl;                                     //avoid collision
 PORTC &= ~scl;                                    //ack clock pulses
 DDRC |= sda;                                      //port output
}

void twiWrite(byte c, byte mod) {
  PORTC |= sda;
  PORTC |= scl;  
  PORTC &= ~sda; 
  PORTC &= ~scl;  //twiStart(); 
